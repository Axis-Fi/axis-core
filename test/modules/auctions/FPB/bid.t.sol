...
contract FpbBidTest is FpbTest {
    ...
    // [X] when the quote token decimals are greater
    //  [X] it records the bid accurately
    // [X] when the quote token decimals are smaller
    //  [X] it records the bid accurately
    ...
    function test_bidsReachCapacity_quoteTokenDecimalsLarger()
        public
        givenQuoteTokenDecimals(21)
        givenBaseTokenDecimals(13)
        givenLotIsCreated
        givenLotHasStarted
    {
        // Create a bid to fill half capacity (10/2 = 5)
        _createBid(_scaleQuoteTokenAmount(10e18));
        ...
        assertEq(auctionData.nextBidId, 3, "nextBidId");
        assertEq(auctionData.settlementCleared, false, "settlementCleared"); // Not settled yet
        assertEq(auctionData.totalBidAmount, _scaleQuoteTokenAmount(20e18), "totalBidAmount");

        // Assert bid one
        IFixedPriceBatch.Bid memory bidData = _module.getBid(_lotId, 1);
        assertEq(bidData.bidder, _BIDDER, "bid one: bidder");
        assertEq(bidData.referrer, _REFERRER, "bid one: referrer");
        assertEq(bidData.amount, _scaleQuoteTokenAmount(10e18), "bid one: amount");
        assertEq(
            uint8(bidData.status), uint8(IFixedPriceBatch.BidStatus.Submitted), "bid one: status"
        );
        ...
        assertEq(bidData.bidder, _BIDDER, "bid two: bidder");
        assertEq(bidData.referrer, _REFERRER, "bid two: referrer");
        assertEq(bidData.amount, _scaleQuoteTokenAmount(10e18), "bid two: amount");
        assertEq(
            uint8(bidData.status), uint8(IFixedPriceBatch.BidStatus.Submitted), "bid two: status"
        );
    ...
    function test_bidsReachCapacity_quoteTokenDecimalsSmaller()
        public
        givenQuoteTokenDecimals(13)
        givenBaseTokenDecimals(21)
        givenLotIsCreated
        givenLotHasStarted
    {
        // Create a bid to fill half capacity (10/2 = 5)
    ...
    function test_bidsOverCapacity_quoteTokenDecimalsLarger()
        public
        givenQuoteTokenDecimals(21)
        givenBaseTokenDecimals(13)
        givenLotIsCreated
        givenLotHasStarted
    {
        ...
        // Create a second bid to fill the over the remaining capacity (12/2 = 6)
        _createBid(_scaleQuoteTokenAmount(12e18));
        ...
        assertEq(auctionData.nextBidId, 3, "nextBidId");
        assertEq(auctionData.settlementCleared, false, "settlementCleared"); // Not settled yet
        assertEq(auctionData.totalBidAmount, _scaleQuoteTokenAmount(20e18), "totalBidAmount"); // Excludes refund
        ...
        assertEq(bidData.bidder, _BIDDER, "bid two: bidder");
        assertEq(bidData.referrer, _REFERRER, "bid two: referrer");
        assertEq(bidData.amount, _scaleQuoteTokenAmount(12e18), "bid two: amount");
        assertEq(
            uint8(bidData.status), uint8(IFixedPriceBatch.BidStatus.Submitted), "bid two: status"
        );
        ...
        assertEq(hasPartialFill, true, "hasPartialFill");
        assertEq(partialFill.bidId, 2, "partialFill: bidId");
        assertEq(partialFill.refund, _scaleQuoteTokenAmount(2e18), "partialFill: refund");
        assertEq(partialFill.payout, _scaleBaseTokenAmount(5e18), "partialFill: payout");
    }

    function test_bidsOverCapacity_quoteTokenDecimalsSmaller()
        public
        givenQuoteTokenDecimals(13)
        givenBaseTokenDecimals(21)
        givenLotIsCreated
    ...
    function test_partialFill_auctionPriceFuzz(uint256 price_) public {
        ...
        uint256 price = bound(price_, 1, type(uint96).max / 10e18);
        _setPrice(price);
