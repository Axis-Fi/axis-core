...
contract MockEMPAHook is IHooks {
    ...
    uint96 public preAuctionCreateMultiplier;
    ...
    function setPreAuctionCreateMultiplier(uint96 multiplier_) external {
    ...
    function preAuctionCreate(uint96 lotId_) external override {
        ...
        (, ERC20 lotBaseToken,,,,,,,,,) =
            EncryptedMarginalPriceAuction(msg.sender).lotRouting(lotId_);

        // Get the capacity
        (, uint96 capacity,,,,,,,) = EncryptedMarginalPriceAuction(msg.sender).lotData(lotId_);
